...
const PreviewBtn = ({ data = [], setFilteredData }) => {
    ...
    // Apply filters whenever selectedNumbers or selectedDates change
    useEffect(() => {
        if (setFilteredData) {
            const filtered = applyAllFilters(data);
            setFilteredData(filtered);
        }
    }, [selectedNumbers, selectedDates]);
    ...
    const handleGlobalSearch = (val) => {
        const filtered = applyAllFilters(data, val);
        setFilteredData && setFilteredData(filtered);
    };
        ...
        <div className="flex items-center justify-end gap-2 relative">
            {!isFilterOpen ? (
                ...
                <div className="w-[115px] h-[41px] flex-shrink-0 rounded-[5.145px] bg-[#598931] border border-gray-300 shadow-lg flex items-center space-x-1 px-2 relative">
                    <button
                        className="p-1 bg-[#F2FFE8] rounded-md"
                        onClick={(e) => { e.preventDefault(); toggleNumberDropdown(); }}
                        title="Number Filter"
                    >
                        <Bs123 className="text-green-900" size={20} />
                    </button>
                    <button
                        className="p-1 bg-[#F2FFE8] rounded-md"
                        onClick={(e) => { e.preventDefault(); toggleDateDropdown(); }}
                        title="Date Filter"
                    >
                        <CiCalendarDate className="text-green-900" size={20} />
                    </button>
            ...
            {isNumberDropdownOpen && (
                <div className="absolute top-full left-[-50px] mt-1 max-w-[260px] bg-white border border-gray-300 shadow-lg rounded-md p-2 z-50 overflow-auto">
                    <div className="flex items-center justify-between gap-2 mb-2">
                        <div className="text-sm font-medium">Number Options</div>
                        <button
                            onClick={(e) => { e.preventDefault(); setIsNumberDropdownOpen(false); }}
                            className="text-gray-500 hover:text-gray-800 focus:outline-none"
                            title="Close"
                        >
                            <ImCancelCircle />
            ...
            {isDateDropdownOpen && (
                <div className="absolute top-full left-[-50px] mt-1 max-w-[260px] bg-white border border-gray-300 shadow-lg rounded-md p-2 z-50 overflow-auto">
                    <div className="flex items-center justify-between gap-2 mb-2">
                        <div className="text-sm font-medium">Date Options</div>
                        <button
                            onClick={(e) => { e.preventDefault(); setIsDateDropdownOpen(false); }}
                            className="text-gray-500 hover:text-gray-800 focus:outline-none"
                            title="Close"
                        >
                            <ImCancelCircle />
            ...
            {(selectedNumbers.length > 0 || selectedDates.length > 0) && (
                // Make the overlay container ignore pointer events so it doesn't block other UI (like opening the other filter)
                <div className="fixed left-1/2 -translate-x-1/2 z-40 px-4 w-full max-w-[1200px] pointer-events-none" style={{ top: '80px' }}>
                    {/* Keep the row wrapper non-interactive; only slider cards will be interactive */}
                    <div className="flex flex-wrap gap-4 justify-center pointer-events-none">
                        {selectedNumbers.map((slider, index) => {
                            const { min, max } = calculate_number_min_max(data, slider.column);
                            return (
                                <div key={`num-${slider.column}-${index}`} className="flex flex-col items-center w-[240px] pointer-events-auto">
                                    <span className="font-medium text-gray-700 mb-2 text-center text-[14px]">
                                        {slider.column.split('_').join(' ').toUpperCase()}
                                    </span>
                                    <div className="flex flex-col items-center w-full relative">
                        ...
                        {selectedDates.map((slider, index) => {
                            ...
                            const maxDate = max || new Date().getTime();
                            return (
                                <div key={`date-${slider.column}-${index}`} className="flex flex-col items-center w-[240px] pointer-events-auto">
                                    <span className="font-medium text-gray-700 mb-2 text-center text-[14px]">
                                        {slider.column.split('_').join(' ').toUpperCase()}
                                    </span>
                                    <div className="flex flex-col items-center w-full relative">
