...
//   const handleAddSave = () => {
//     if (inputValue.trim() !== "") {
//       let newBoxValue = [...fields, { id: Date.now().toString(), text: inputValue, value: `<h2 className="text-lg font-bold mb-2"></h2>` }];
            ...
            >
              {fields.map((field, index) => (
                <Draggable key={field.id} draggableId={field.id} index={index}>
                  {(provided, snapshot) => (
                    <div
                      className="relative group mr-10 ml-4"
                    >
                      <div
                        ref={provided.innerRef}
                        {...provided.draggableProps}
              ...
              <button
                onClick={addField}
                className="inline-flex px-4 py-2 justify-center items-center gap-[18px] border-[3px] border-[#598931] rounded-[8px] text-[#598931] hover:bg-[#E6F4D7] transition ml-4"
              >
                <span className="text-[16px] font-medium font-[Poppins]">
                  +
                </span>
